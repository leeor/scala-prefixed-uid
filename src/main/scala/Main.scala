...
given HasPrefix[Options] with
  extension(o: Options) def getPrefix: String =
    o.prefix

given HasTimePrecision[Options] with
  extension(o: Options) def getTimePrecision: TimePrecision =
    o.timePrecision

given HasRandomLength[Options] with
  extension(o: Options) def getRandomLength: Int =
    o.randomLength
...
def encode(i: Long, encoded: String = ""): String =
  if (i == 0) {
    ...
    encode(i / charsetSize, charset((i % charsetSize).toInt) + encoded)
  }
...
def getRandomPart[E: HasRandomLength]: Reader[E, String] = Reader(e => 
    Random.alphanumeric.take(e.getRandomLength).toList.mkString
    )
...
object UIDApp extends CommandIOApp(
  name = "sc-uid",
  ...
  override def main: Opts[IO[ExitCode]] =
    ...
    val randomLengthOpt = Opts.option[Int]("random-length", short = "l", help = "number of random characters").withDefault(16)
    val timePrecisionOpt = Opts.option[TimePrecision]("time-precision", short = "t", help = "time precision").withDefault(Millisecond)
